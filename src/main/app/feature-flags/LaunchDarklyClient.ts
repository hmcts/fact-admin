...
export class LaunchDarkly implements FeatureFlagClient {
  ...
  public onFlagChange(callback: Function, defaultValue: boolean, flag?: string): void {
    if(flag) {
      this.client.on(`update:${flag}` , async ():Promise<void> => callback(await this.getFlagValue(flag, defaultValue)));
    } else {
      this.client.on('update' , async ():Promise<void> => callback(await this.getAllFlagValues(defaultValue)));
    }
  }
