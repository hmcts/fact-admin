...
describe('FactApi', () => {
  ...
  test('Should return results from getContacts request', async () => {
    const results: { data: Contact[] } = {
      data: [
        ...
        {
          'type_id': 43,
          number: '0987 6060 5050',
          fax: true,
          explanation: 'Explanation',
          'explanation_cy': 'Exp in Welsh'
        }
      ]
    };
    const mockAxios = {get: async () => results} as any;
      ...
      data: [
        {
          ...
          'address_lines': ['100 Green Street'],
          'address_lines_cy': [],
          town: 'Red Town',
          'town_cy': '',
          'county_id': 1,
        ...
        },
        {
          ...
          description: 'description',
          'description_cy': 'description_cy',
          'address_lines': ['122 Green Street'],
          'address_lines_cy': [],
          town: 'Red Town',
          'town_cy': '',
          'county_id': 1,
  ...
  test('Should return audits', async () => {
    const results: { data: Audit[] } = {
      data: [
        {
          id: 1,
          action: {name: 'test', id: 1} as Action,
          creation_time: 'a time',
          action_data_after: 'data after',
          action_data_before: 'data before',
          location: 'location',
  ...
  test('Should log error and reject promise for failed createOpeningType request ', async () => {
    ...
    await expect(api.createOpeningType({
      id: null,
      type: 'Opening 1',
      'type_cy': 'Opening 1 cy'
    })).rejects.toBe(mockError);
    expect(loggerSpy).toBeCalled();
  });
  ...
  test('Should log error and reject promise for failed updateOpeningType request ', async () => {
    ...
    await expect(api.updateOpeningType({
      id: 4321,
      type: 'Opening 1',
      'type_cy': 'Opening 1 cy'
    })).rejects.toBe(mockError);
      ...
      data: [
        {
          ...
          external_link_description_cy: 'external_link_description_cy'
        },
        {
          type: 'type_2',
          type_cy: 'type_cy',
  ...
  test('Should log error and reject promise for failed deleteCourtLocks request', async () => {
    const mockAxios = {
      delete: async () => {
        throw mockError;
      }
    ...
    const spy = jest.spyOn(mockLogger, 'info');
    const api = new FactApi(mockAxios, mockLogger);
    await expect(api.deleteCourtLocks('newcastle-crown-court', 'moshuser')).rejects.toBe(mockError);
    await expect(spy).toBeCalled();
  });
  ...
  test('Should return results from deleteCourtLocks request', async () => {
    ...
    await expect(api.deleteCourtLocks('newcastle-crown-court', 'moshuser')).resolves.toEqual(results.data);
  });

  test('Should log error and reject promise for failed deleteCourtLocksByEmail request', async () => {
    const mockAxios = {
      delete: async () => {
        throw mockError;
      }
    ...
    const spy = jest.spyOn(mockLogger, 'info');
    const api = new FactApi(mockAxios, mockLogger);
    await expect(api.deleteCourtLocksByEmail('moshuser')).rejects.toBe(mockError);
    await expect(spy).toBeCalled();
  });
  ...
  test('Should return results from deleteCourtLocksByEmail request', async () => {
    ...
    await expect(api.deleteCourtLocksByEmail('moshuser')).resolves.toEqual(results.data);
  });

  test('Should log error and reject promise for failed addCourtLock request', async () => {
    const mockAxios = {
      post: async () => {
        throw mockError;
      }
  ...
  test('Should return results from addCourtLock request', async () => {
    ...
    await expect(api.addCourtLock('moshuser', results.data[0])).resolves.toEqual(results.data);
  });

  test('Should log error and reject promise for failed getCourtLocks request', async () => {
    const mockAxios = {
      get: async () => {
        throw mockError;
      }
    ...
    const spy = jest.spyOn(mockLogger, 'info');
    const api = new FactApi(mockAxios, mockLogger);
    await expect(api.getCourtLocks('kupo-court-slug')).rejects.toBe(mockError);
    await expect(spy).toBeCalled();
  });
