...
describe('IdamApi', () => {
  ...
  test('Should post register user request', async () => {
    ...
    await expect(api.registerUser(result.data,'access_token')).resolves.toEqual(result.data);
  });
  ...
  test('Should log error and reject promise for failed register user request', async () => {
    ...
    await expect(api.registerUser(result.data, 'access_token')).rejects.toBe(mockError);
  });
