...
async function checkAndClearAddressField(addressFieldElement: string) {
  expect(await I.checkElement(addressFieldElement, 30000)).equal(true);
...
Given('I will make sure to clear all entries of third address', async () => {
  const elementExist = await I.checkElement('button[name="removeThirdAddress"]', 30000);
  expect(elementExist).equal(true);
...
Then('I select children and civil from area of law and county court for court type', async () => {
  ...
  expect(await I.checkElement(selectorAolChildren, 30000)).equal(true);
  await I.click(selectorAolChildren);
  expect(await I.checkElement(selectorAolCivil, 30000)).equal(true);
  await I.click(selectorAolCivil);
  expect(await I.checkElement(selectorCountyCourt, 30000)).equal(true);
